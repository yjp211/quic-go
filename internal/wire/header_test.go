...
var _ = Describe("Header Parsing", func() {
	...
	Context("Long Headers", func() {
		It("parses a Long Header", func() {
			...
			Expect(hdr.SrcConnectionID).To(Equal(srcConnID))
			b := bytes.NewReader(data)
			extHdr, err := hdr.ParseExtended(b, protocol.PerspectiveServer, versionIETFFrames)
			Expect(err).ToNot(HaveOccurred())
		...
		It("rejects packets sent with an unknown packet type", func() {
			...
			}).Write(buf, protocol.PerspectiveClient, protocol.VersionTLS)).To(Succeed())
			b := bytes.NewReader(buf.Bytes())
			hdr, err := ParseHeader(b, 0)
			Expect(err).ToNot(HaveOccurred())
			_, err = hdr.ParseExtended(b, protocol.PerspectiveClient, versionIETFFrames)
		...
		It("errors if the token length is too large", func() {
			...
			hdr, err := ParseHeader(b, 0)
			Expect(err).ToNot(HaveOccurred())
			_, err = hdr.ParseExtended(b, protocol.PerspectiveServer, versionIETFFrames)
	...
	Context("Short Headers", func() {
		It("reads a Short Header with a 8 byte connection ID", func() {
			...
			b := bytes.NewReader(data)
			extHdr, err := hdr.ParseExtended(b, protocol.PerspectiveClient, versionIETFFrames)
			Expect(err).ToNot(HaveOccurred())
